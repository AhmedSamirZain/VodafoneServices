# ======================================================================
#  [النسخة المعززة أمنياً] — أبرز التغييرات عن الأصل:
#  1) الأسرار كلها في .env (عبر config.py) — لا يوجد أي سر داخل الكود
#  2) كلمات المرور المحفوظة مشفرة Fernet بدل plaintext
#  3) قاعدة البيانات: اتصال وقائم خاص بكل خيط (لم يعد هناك cursor مشترك)
#  4) قفل STATE_LOCK على حالة الجلسات المشتركة بين الخيوط
...
# [SECURITY] أدوات الأمان المضافة (خزنة مشفرة، تقييد معدل، قفل حالة، سجل تدقيق)
from security import (
    PasswordVault, RateLimiter, STATE_LOCK, audit,
    is_egyptian_mobile, sanitize_text,
)
...
# [SECURITY] كلمة المرور محفوظة مشفرة (Fernet) وليس plaintext
# {user_id: {"phone": str, "password_enc": str, "saved_at": datetime, "expires_at": datetime}}
_saved_passwords_vault = PasswordVault(VAULT_KEY)
login_limiter = RateLimiter(max_hits=5, window_seconds=15 * 60)  # 5 محاولات دخول / 15 دقيقة
saved_passwords = {}

def save_password_for_user(user_id: int, phone: str, password: str):
    """حفظ بيانات تسجيل الدخول للمستخدم لمدة 24 ساعة (كلمة المرور مشفرة)"""
    now = datetime.now()
    with STATE_LOCK:
        saved_passwords[user_id] = {
            "phone": phone,
            "password_enc": _saved_passwords_vault.seal(password),
            "saved_at": now,
            "expires_at": now + timedelta(hours=24)
        }

def get_saved_password(user_id: int) -> Optional[Dict]:
    """جلب البيانات المحفوظة إذا لم تنتهِ صلاحيتها (مع فك تشفير كلمة المرور)"""
    with STATE_LOCK:
        if user_id not in saved_passwords:
            return None
        data = dict(saved_passwords[user_id])
        if datetime.now() > data.get("expires_at", datetime.min):
            del saved_passwords[user_id]
            return None
    try:
        data["password"] = _saved_passwords_vault.open(data["password_enc"])
        return data
    except Exception:
        # النص المشفر تالف أو المفتاح تغيّر → نتعامل كأنه غير محفوظ
        with STATE_LOCK:
            saved_passwords.pop(user_id, None)
        return None
...
def refresh_phone_token_if_needed(phone: str) -> Tuple[bool, Optional[str]]:
    """التحقق من صلاحية توكن الرقم وتجديده إذا لزم الأمر [SECURITY: القراءة/الكتابة تحت القفل، واتصال الشبكة خارجه]"""
    with STATE_LOCK:
        if phone not in phone_tokens:
            return False, None
        token_data = dict(phone_tokens[phone])
    ...
    # محاولة تجديد التوكن (اتصال شبكة — خارج القفل)
...
def update_session_token(user_id: int, new_bearer_token: str, new_token: str):
    """تحديث التوكن في جلسة المستخدم [SECURITY: التعديل تحت القفل]"""
    with STATE_LOCK:
...
def check_and_refresh_user_session(user_id: int) -> bool:
    """التحقق من صلاحية جلسة المستخدم وتجديدها إذا لزم الأمر [SECURITY: القفل للبيانات فقط، وطلبات الشبكة خارجه]"""
    with STATE_LOCK:
        ...
        if session.get('expires_at') and session['expires_at'] < datetime.now():
            ...
            return False
        phone = session['phone']
        current_bearer = session.get('bearer_token', '')
    ...
    # عن طريق اختبار بسيط (اتصال شبكة — خارج القفل)
    try:
        test_url = "https://mobile.vodafone.com.eg/services/dxl/usage/usageConsumptionReport"
        test_params = {'@type': "aggregated", 'bucket.product.publicIdentifier': phone}
        test_headers = {
        ...
        if test_response.status_code == 200:
            ...
            with STATE_LOCK:
                if user_id in user_sessions:
                    user_sessions[user_id]['expires_at'] = datetime.now() + timedelta(minutes=25)
            return True
    except:
        pass
...
def create_session(user_id: int, phone: str, password: str, token: str, bearer_token: str):
    """إنشاء جلسة جديدة للمستخدم [SECURITY: تحت القفل + تسجيل العملية في audit.log]"""
    now = datetime.now()
    with STATE_LOCK:
    ...
    audit.log("login_success", user_id, phone)
    login_limiter.reset(user_id)


def get_session_by_phone(phone: str) -> Optional[Dict]:
    """البحث عن جلسة نشطة بواسطة رقم الهاتف [SECURITY: تحت القفل]"""
    with STATE_LOCK:
...
def auto_refresh_all_tokens():
    ...
    [SECURITY: نجمع الأرقام تحت القفل، ثم نعمل الشبكة خارجه، ثم نطبق التحديثات تحت القفل —
    حتى لا يتعطل البوت كله أثناء اتصال شبكة بطيء]"""
    while True:
        try:
            ...
            # البحث عن الأرقام التي تحتاج تجديد (قراءة سريعة تحت القفل)
            with STATE_LOCK:
            ...
            # تجديد التوكنات (اتصالات شبكة — خارج القفل)
...
def _voda_login_for_line_data(phone: str, password: str) -> dict:
    ...
    attempts = [
        (
            ...
            {'client_id': 'ana-vodafone-app', 'client_secret': VODA_CLIENT_SECRET},
        ),
        (
            {
...
# DB_FILE و DELETE_OLD_DB_ON_START يُحمّلان الآن من config.py (.env)
...
def get_call_history(phone, password, days_back=30):
    """جلب سجل المكالمات لآخر عدد من الأيام"""
    try:
        # تسجيل الدخول
        auth_result = get_authorization(phone, password)
        if not auth_result['success']:
        ...
        # بناء URL يدويًا لتجنب encoding لـ @ و $
        raw_url = (
            f"https://mobile.vodafone.com.eg/services/dxl/usagemng/usage"
            f"?relatedParty.id={phone}"
        ...
        if response.status_code != 200:
            return False, f"❌ فشل جلب السجل (كود {response.status_code})"
        ...
        if isinstance(calls_raw, list):
            for item in calls_raw:
                ...
                duration_s = ""
                call_dir = ""
                for c in item.get("usageCharacteristic", []):
                    n = c.get("name", "")
                    v = c.get("value", "")
                    if n == "dialedNumber":
                        number = v
                ...
                calls.append({"number": number or "غير معروف", "duration": dur, "date": date_fmt, "dir": call_dir})
...
class Database:
    def __init__(self):
        ...
        # [SECURITY/STABILITY] كل خيط يستخدم اتصالاً وقائماً (cursor) خاصين به.
        # النسخة الأصلية كانت cursor واحد مشترك بين خيطي البوتين وخيط التجديد
        # → تعارض نتائج واستعلامات متقاطعة وأخطاء "recursive use of cursors".
    ...
    def _bootstrap(self):
        """تهيئة قاعدة البيانات (جداول + بيانات أولية) مرة واحدة فقط"""
        with self._init_lock:
            ...
            self._get_cursor().connection.commit()
            self._bootstrapped = True

    def _get_cursor(self):
        """إرجاع الـ cursor الخاص بالخيط الحالي (إنشاء اتصال جديد عند أول استخدام)"""
        if not hasattr(self._local, "cursor"):
            self._local.conn = sqlite3.connect(DB_FILE, check_same_thread=False)
    ...
    def is_database_empty(self):
        try:
            self._get_cursor().execute("SELECT COUNT(*) FROM levels WHERE parent_id = 0")
            count = self._get_cursor().fetchone()[0]
            return count == 0
        except:
            return True
    ...
    def add_missing_buttons(self):
        ...
        if manage_id:
            ...
            try:
                self._get_cursor().execute("SELECT id FROM levels WHERE name = 'transfer_flex' AND parent_id = ?", (manage_id,))
                transfer_flex_in_manage = self._get_cursor().fetchone()
            except:
                pass
        ...
        if packages_id:
            ...
            if extend_item:
                try:
                    self._get_cursor().execute("DELETE FROM levels WHERE name = 'extend'")
                    self._get_cursor().connection.commit()
        ...
        if system_details:
            try:
                self._get_cursor().execute("DELETE FROM levels WHERE name = 'system_details'")
                self._get_cursor().connection.commit()
        ...
        if old_charge_card:
            try:
                self._get_cursor().execute("DELETE FROM levels WHERE name = 'charge_card'")
                self._get_cursor().connection.commit()
        ...
        try:
            ...
            if renew_item and delete_debt_item:
                renew_pos = None
                delete_pos = None
                self._get_cursor().execute("SELECT position FROM levels WHERE name = 'renew'")
                r = self._get_cursor().fetchone()
                if r: renew_pos = r[0]
                self._get_cursor().execute("SELECT position FROM levels WHERE name = 'delete_debt_21000'")
                r = self._get_cursor().fetchone()
                if r: delete_pos = r[0]
                # إذا كان renew قبل delete_debt_21000، نعكسهما
                if renew_pos is not None and delete_pos is not None and renew_pos < delete_pos:
                    self._get_cursor().execute("UPDATE levels SET position = ? WHERE name = 'renew'", (delete_pos,))
                    self._get_cursor().execute("UPDATE levels SET position = ? WHERE name = 'delete_debt_21000'", (renew_pos,))
                    self._get_cursor().connection.commit()
    ...
    def init_default_data(self):
        ...
        level1_data = [
            ...
            ("other_section", f"{EMOJI['other']} الخدمات الأخرى", 0, "section", EMOJI['other'], None, 5)
        ]
    ...
    def get_id_by_name(self, name):
        try:
            self._get_cursor().execute("SELECT id FROM levels WHERE name = ?", (name,))
    ...
    def get_users_count(self):
        try:
            self._get_cursor().execute("SELECT COUNT(*) FROM users")
    ...
    def get_today_users_count(self):
        try:
            ...
            self._get_cursor().execute("SELECT COUNT(*) FROM users WHERE DATE(last_active) = ?", (today,))
            return self._get_cursor().fetchone()[0]
        except:
            return 0
    ...
    def get_month_users_count(self):
        try:
            ...
            self._get_cursor().execute("SELECT COUNT(*) FROM users WHERE strftime('%Y-%m', last_active) = ?", (month,))
            return self._get_cursor().fetchone()[0]
        except:
            return 0
    ...
    def get_all_users(self):
        try:
            self._get_cursor().execute("SELECT user_id FROM users")
            return [row[0] for row in self._get_cursor().fetchall()]
        except:
            return []
    ...
    def update_item_status(self, item_id, field, value):
        try:
            self._get_cursor().execute(f"UPDATE levels SET {field} = ? WHERE id = ?", (value, item_id))
            self._get_cursor().connection.commit()
            return True
        except:
            return False
    ...
    def update_item_content(self, item_id, content):
        try:
            self._get_cursor().execute("UPDATE levels SET content = ? WHERE id = ?", (content, item_id))
            self._get_cursor().connection.commit()
            return True
        except:
            return False
    ...
    def delete_item(self, item_id):
        try:
            ...
            self._get_cursor().execute("SELECT id FROM levels WHERE parent_id = ?", (item_id,))
            children = [r[0] for r in self._get_cursor().fetchall()]
            for child_id in children:
                self.delete_item(child_id)
            self._get_cursor().execute("DELETE FROM levels WHERE id = ?", (item_id,))
            self._get_cursor().connection.commit()
            return True
        except:
            return False
    ...
    def is_user_subscribed(self, user_id):
        ...
        try:
            ...
            return self._get_cursor().fetchone() is not None
        except:
            return False
    ...
    def add_subscription(self, user_id, username, first_name, months=1):
        ...
        self._get_cursor().execute("SELECT end_date FROM subscriptions WHERE user_id = ? AND is_active = 1", (user_id,))
    ...
    def remove_subscription(self, user_id):
        """حذف اشتراك مستخدم"""
        try:
            self._get_cursor().execute("DELETE FROM subscriptions WHERE user_id = ?", (user_id,))
            self._get_cursor().connection.commit()
            return True
        except:
            return False
    ...
    def get_subscription_info(self, user_id):
        ...
        try:
            self._get_cursor().execute("SELECT * FROM subscriptions WHERE user_id = ?", (user_id,))
            return self._get_cursor().fetchone()
        except:
            return None
    ...
    def get_all_subscribed_users(self):
        ...
        try:
            now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            self._get_cursor().execute("SELECT user_id, username, first_name, end_date FROM subscriptions WHERE is_active = 1 AND end_date >= ?", (now,))
            return self._get_cursor().fetchall()
        except:
            return []
    ...
    def is_user_blocked(self, user_id):
        """هل المستخدم محظور"""
        try:
            self._get_cursor().execute("SELECT is_blocked FROM users WHERE user_id = ?", (user_id,))
            row = self._get_cursor().fetchone()
    ...
    def block_user(self, user_id):
        """حظر مستخدم"""
        try:
            self._get_cursor().execute("UPDATE users SET is_blocked = 1 WHERE user_id = ?", (user_id,))
            self._get_cursor().connection.commit()
            return True
        except:
            return False
    ...
    def unblock_user(self, user_id):
        """إلغاء حظر مستخدم"""
        try:
            self._get_cursor().execute("UPDATE users SET is_blocked = 0 WHERE user_id = ?", (user_id,))
            self._get_cursor().connection.commit()
            return True
        except:
            return False
...
def handle_login(message, user_id):
    ...
    if action == 'login_waiting_phone':
        ...
        # [SECURITY] تقييد محاولات تسجيل الدخول (5 محاولات / 15 دقيقة) ضد التخمين
        if not login_limiter.allow(user_id):
            wait_min = max(1, int(login_limiter.retry_after(user_id) // 60))
            user_bot.send_message(
                user_id,
                f"{EMOJI['error']} *تم تجاوز عدد المحاولات المسموح!*\nيرجى المحاولة مرة أخرى بعد {wait_min} دقائق.",
                parse_mode='Markdown'
            )
            return
...
def start_command(message):
    ...
    user_bot.send_message(
        message.chat.id,
        f"👋 أهلاً بك *{first_name}* في بوت 𝐁𝐑𝐒𝐇𝐀𝐌𝐇 𝐅𝐋𝐄𝐗 💪🏻\n\n اختر أحد الخيارات:",
        parse_mode='Markdown',
        reply_markup=markup
    )
...
def handle_callbacks(call):
    ...
    if data.startswith("confirm_sub_"):
        ...
        # جلب بيانات الطلب
        db._get_cursor().execute("SELECT user_id, username, first_name FROM subscription_requests WHERE id = ?", (req_id,))
        row = db._get_cursor().fetchone()
        if row:
            sub_user_id, sub_username, sub_first_name = row
            end_date = db.add_subscription(sub_user_id, sub_username, sub_first_name, months=1)
            audit.log("subscription_approved", sub_user_id, f"req={req_id}")
            db._get_cursor().execute("UPDATE subscription_requests SET status = 'approved' WHERE id = ?", (req_id,))
            db._get_cursor().connection.commit()
            try:
    ...
    if data.startswith("reject_sub_"):
        req_id = int(data.split("_")[2])
        db._get_cursor().execute("SELECT user_id, first_name FROM subscription_requests WHERE id = ?", (req_id,))
        row = db._get_cursor().fetchone()
        if row:
            sub_user_id, sub_first_name = row
            audit.log("subscription_rejected", sub_user_id, f"req={req_id}")
            db._get_cursor().execute("UPDATE subscription_requests SET status = 'rejected' WHERE id = ?", (req_id,))
            db._get_cursor().connection.commit()
            try:
                user_bot.edit_message_caption(
...
def admin_handle_text(message):
    ...
    if bot_status.get('admin_action') == 'waiting_broadcast':
        ...
        for uid in users:
            try:
                try:
                    ...
                    # [SECURITY/UX] لو الرسالة فيها Markdown غير صالح، نعيد إرسالها نصاً عادياً
                    user_bot.send_message(uid, f"{EMOJI['broadcast']} رسالة من الإدارة\n\n{text}")
                sent += 1
                time.sleep(0.05)
            except Exception:
        ...
        audit.log("broadcast", "admin", f"sent={sent}/{len(users)}")
        ...
        try:
            ...
            audit.log("user_blocked", target_id, "by_admin")
        ...
        try:
            ...
            audit.log("user_unblocked", target_id, "by_admin")
...
def get_status() -> Dict:
    """حالة البوت (لو عايز صفحة ستريمليت تعرض حالة التشغيل)"""
    return {
...
def _in_streamlit() -> bool:
    """هل نحن جوا تنفيذ ستريمليت فعلي؟ (مش مجرد إن المكتبة متسطبة)"""
    try:
...
def main():
    """
    نقطة التشغيل:
    - جوا ستريمليت: بينشغل البوت في الخلفية ويرجع فوراً (الصفحة تفضل شغالة).
    - كونسول عادي (python stream.py): بينشغل ويبلّكس لحد Ctrl+C.
    """
    if start():
    ...
    if _in_streamlit():
        return  # ستريمليت هو اللي بيفضل شغال — مينفعش نبلّكس عليه
